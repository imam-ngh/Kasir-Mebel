...
export default async function handler(request, response) {
    ...
    try {
        switch (request.method) {
            case 'GET': {
                const { rows } = await client.query('SELECT * FROM transactions ORDER BY date DESC;');
            ...
            case 'POST': {
                ...
                for (const item of transaction.items) {
                    if (!item.productId) {
                        // Ini bisa terjadi jika ada item di keranjang yang tidak memiliki productId (misal: produk yang dihapus)
                        // Anda mungkin ingin menangani ini lebih baik, misal dengan log warning atau error
                        console.warn(`Item tanpa productId ditemukan: ${item.name}`);
                        continue;
                    }
